  ...
  <section class="py-16">
    <UContainer>
      ...
      >
        <div>
          <span
            class="text-secondary font-semibold tracking-wide uppercase text-sm"
            >{{ $t("news.subtitle") }}</span
          >
          <h2
            class="mt-2 text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white"
          >
            {{ $t("news.title") }}
          </h2>
        </div>
      ...
      <div
        class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5 sm:gap-6 lg:gap-8"
      >
        <!-- Featured article -->
        <div
        ...
        >
          <div
            class="h-60 sm:h-72 lg:h-96 bg-slate-900 relative flex items-end"
          >
            ...
            <div class="relative p-5 sm:p-6 lg:p-8 w-full">
              ...
              <h3
                class="text-lg sm:text-xl lg:text-2xl font-bold text-white mb-1 sm:mb-2"
              >
                {{ articles[0]?.title }}
              </h3>
              <p
                class="text-sm text-white/70 mb-2 sm:mb-3 line-clamp-2 hidden sm:block"
              >
                {{ articles[0]?.excerpt }}
              </p>
              <div class="flex items-center justify-between">
                ...
                <div
                  class="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-white/10 backdrop-blur-sm flex items-center justify-center text-white"
                >
                  <UIcon
        ...
        <div class="sm:col-span-2 lg:col-span-2 flex flex-col gap-4 sm:gap-5">
          ...
          >
            <!-- Compact visual -->
            <div
              class="w-24 sm:w-28 lg:w-32 shrink-0 bg-slate-900 relative overflow-hidden"
            >
            ...
            <div class="p-3 sm:p-4 flex flex-col justify-center flex-1 min-w-0">
              ...
              <h3
                class="text-sm sm:text-base font-bold text-slate-900 dark:text-white mb-1 line-clamp-2 group-hover:text-primary transition-colors"
              >
                {{ article.title }}
              </h3>
...
const BarChartVisual = () =>
  h("div", { class: "absolute inset-0" }, [
    ...
    h(
      ...
      {
        class:
          "absolute inset-x-6 bottom-6 h-24 flex items-end gap-2 z-0 opacity-60",
      },
      [
...
const SpinnerVisual = () =>
  h(
    ...
    h("div", {
      class:
        "w-16 h-16 rounded-full border-4 border-t-primary border-r-secondary border-b-blue-400 border-l-white animate-spin",
    })
  );
