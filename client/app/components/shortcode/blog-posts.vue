    ...
    <UContainer>
      ...
      <div v-if="isListingMode" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <NuxtLink
          ...
          :key="article.id"
          :to="article.url || '/blog'"
          class="card-hover-glow rounded-2xl overflow-hidden group cursor-pointer border border-slate-100 bg-white shadow-sm flex flex-col"
        >
          <!-- Thumbnail -->
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5 sm:gap-6 lg:gap-8">
        ...
        <div class="sm:col-span-2 lg:col-span-2 flex flex-col gap-4 sm:gap-5">
          <NuxtLink
            ...
            :key="article.id"
            :to="article.url || '/blog'"
            class="card-hover-glow rounded-2xl overflow-hidden flex flex-row group cursor-pointer border border-slate-100 bg-white shadow-sm flex-1"
          >
