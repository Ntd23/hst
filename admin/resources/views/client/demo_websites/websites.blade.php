        ...
        <div class="col-xl-4 col-lg-6 col-md-10">
            <div class="blog-post-item shine-animate-item web">
                <div class="image-scroll-wrapper">
                    <img src="{{ url('/') }}/storage/{{ $website->img_full }}" data-bb-lazy="true" loading="lazy"
                        data-src="{{ url('/') }}/storage/news/1411-1-480x480.jpg"
                        alt="{{ $website->translationByLang($currentLocale)->name ?? $website->name }}"
                        data-ll-status="loaded" class="entered loaded scroll-on-hover">
                </div>
                <div class="blog-post-content pt-3">
                    <h2 class="title">
                        <a class="truncate-2-custom"
                            title="{{ $website->translationByLang($currentLocale)->name ?? $website->name }}"
                            href="{{ route($routeName, $routeParams) }}">
                            {{ $website->translationByLang($currentLocale)->name ?? $website->name }}
                        </a>
                    </h2>
                    ...
                    <div class="blog-post-meta">
                        <ul class="list-wrap">
                            <li>
                                ...
                                {{-- <a href="{{ route('websites.show', $website->slug) }}" class="btn">Chi tiết</a> --}}
                            </li>
